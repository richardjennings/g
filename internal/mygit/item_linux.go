...
func (wdf *wdFile) toIndexItem() (*indexItem, error) {
	...
	item.CTimeS = uint32(wdf.finfo.Sys().(*syscall.Stat_t).Ctim.Sec)
	item.CTimeN = uint32(wdf.finfo.Sys().(*syscall.Stat_t).Ctim.Nsec)
	item.MTimeS = uint32(wdf.finfo.Sys().(*syscall.Stat_t).Mtim.Sec)
	item.MTimeN = uint32(wdf.finfo.Sys().(*syscall.Stat_t).Mtim.Nsec)
